...
func (uid UID) String() string {
	value := uint64(uid.localID)<<28 | uint64(uid.objectType<<18) | uint64(uid.shardID)<<0
	return base58.Encode([]byte(fmt.Sprintf("%v", value)))
}
...
func DecomposeUID(s string) (UID, error) {
	...
	if (1 << 18) > uid {
		return UID{}, fmt.Errorf("invalid object type")
	}
	u := UID{
		localID:    uint32(uid >> 28),
		objectType: int(uid >> 18 & 0x3FF),
		shardID:    uint32(uid >> 0 & 0x3FF),
	}
	return u, nil
}
...
func (uid *UID) Scan(value interface{}) error {
	...
	case []byte:
		...
		i = uint32(a)
	default:
		return errors.New("invalid Scan Sourc")
	}
	*uid = NewUID(i, 0, 1)
	return nil
